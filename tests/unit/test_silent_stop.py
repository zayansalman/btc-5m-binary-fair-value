...
notifies no one, and the persisted ``btc_bot.state`` is left reading "running"
...
import db as _db
from btc_bot.controller import get_status, is_silent_stop
...
async def test_silent_stop_notifies_once_and_heals(bot_db, monkeypatch) -> None:
    ...
    await _db.set_config("btc_bot.state", "running")
    await _db.set_config("btc_bot.updated_at", "2026-07-07T06:40:00+00:00")
...
async def test_clean_stop_never_notifies(bot_db, monkeypatch) -> None:
    ...
    monkeypatch.setattr(controller, "_is_runner_alive", lambda: False)
    await _db.set_config("btc_bot.state", "stopped")
