...
- **Remaining schedule (AMENDED 07-10 later — operator: "$2–4/day is worth it, think in % terms")**: (1) ~2026-07-17: ONE f45 verdict check (≈56 settled trades) via race_status.py; (2) **if f45 CLEARS the bar (CI>0, n≥56, sign-consistent halves) → DEPLOY LIVE SMALL** with existing rails ($3/trade cap, $10 daily trailing halt, kill switch) — operator pre-approved the economics: ~$3–4/day on ≤$100 employed ≈ triple-digit monthly % in a capacity niche too small for professionals; (3) if it FAILS → stop the bot and archive; (4) 2026-08-27 sunset = hard backstop. The earlier "archive regardless" wording imposed the agent's valuation on the operator's utility — corrected.
