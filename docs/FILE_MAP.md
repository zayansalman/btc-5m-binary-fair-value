...
|---|---|---|---|
| `btc_5m_fv/__init__.py` | DEAD? | 0 | BTC 5m Binary Fair Value trading system. |
| `btc_5m_fv/backtest/__init__.py` | DEAD? | 0 | Backtesting harness and metrics. |
...
| `btc_5m_fv/connectors/__init__.py` | DEAD? | 0 | Exchange and data connectors. |
...
| `btc_5m_fv/core/__init__.py` | DEAD? | 0 | Core domain types, interfaces, and exceptions. |
...
| `btc_5m_fv/execution/__init__.py` | DEAD? | 0 | Paper and live execution managers. |
...
| `btc_5m_fv/ops/__init__.py` | DEAD? | 0 | Operator controls and telemetry. |
| `btc_5m_fv/ops/controller.py` | DEAD? | 0 | Unified bot controller — tick loop using execution manager + risk service. |
| `btc_5m_fv/ops/dashboard/__init__.py` | DEAD? | 0 | FastAPI dashboard for BTC 5m Binary Fair Value trading system. |
...
| `btc_5m_fv/ops/dashboard/panels/__init__.py` | WIRED | 1 | Dashboard panels. |
...
| `btc_5m_fv/storage/__init__.py` | DEAD? | 0 | Persistence layer — database, recording, and replay. |
...
| `btc_5m_fv/strategy/__init__.py` | DEAD? | 0 | Signal generation module — fair value, sizing, and signal composition. |
...
| `btc_bot/__init__.py` | WIRED | 4 | BTC 5-minute paper-trading package. |
...
| `btc_bot/calibration_fit.py` | DEAD? | 0 | Fit the side-relative probability calibrator from the closed-trade journal. |
...
| `btc_bot/params_apply.py` | DEAD? | 0 | Layer 2 — operator-gated promotion of proposed -> active strategy params. |
| `btc_bot/params_propose.py` | DEAD? | 0 | Layer 2 — propose tuned strategy parameters from the existing backtest grid. |
...
| `main.py` | WIRED | 19 | Entrypoint for the BTC 5-minute paper trading system. |
| `tools/backtest_btc_strategy.py` | DEAD? | 0 | Run the BTC strategy backtest and parameter optimizer. |
| `tools/chainlink_lead_lag.py` | DEAD? | 0 | Chainlink-vs-Binance BTC lead-lag analysis (issue #57). |
| `tools/clear_auto_pause.py` | DEAD? | 0 | Clear the adaptive auto-pause and resume entries (#36). |
| `tools/demo_snapshot.py` | DEAD? | 0 | Print a BTC paper trading snapshot. |
| `tools/fetch_polymarket_trades.py` | DEAD? | 0 | Pull this account's Polymarket trade history via the CLOB API → CSV. |
| `tools/gen_docs.py` | DEAD? | 0 | Generate the machine-derived sections of the agent docs. |
| `tools/live_detect_wallet.py` | DEAD? | 0 | Detect the Polymarket funder wallet + signature type from a signer key (#34). |
| `tools/live_preflight.py` | DEAD? | 0 | Live-launch preflight: verify the .env wallet config end to end (issue #32). |
| `tools/live_setup.py` | DEAD? | 0 | One-time live-trading onboarding (issues #32, #33). |
| `tools/offline_replay.py` | DEAD? | 0 | Offline replay of the BTC 5-m fair-value strategy on HF Polymarket data. |
